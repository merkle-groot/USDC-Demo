...
contract StakingContract is Ownable {
   ...
   */
  constructor(IERC20 tokenAddress, address treasuryAddress) {
  ...
  function stake(uint256 amount) external {
    ...
    staked[msg.sender] = StakeDetails({
      stakedAmount: amount - fee,
  ...
  function unStake(uint256 amount) external {
    ...
    require(
      block.timestamp - stakeDetails.stakedTimestamp >= 2 hours,
      "Wait until unlock period"
    );
    require(
      stakeDetails.stakedAmount >= amount,
      "Cannot unstake more than the deposited amount."
    );
