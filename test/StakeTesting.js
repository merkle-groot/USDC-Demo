...
describe('Staking', () => {
  ...
  describe('Staking Related', () => {
    it('Should initialize the right USDC contract', async () => {
      const tokenAddress = await hardhatStaking.erc20Address();
      expect(hardhatUSDC.address).to.be.equal(tokenAddress);
    });

    it('Should be able to stake required amount of tokens', async () => {
      await hardhatUSDC
        .connect(addr1)
    ...
    it('Should collect right amount of fee', async () => {
      await hardhatUSDC
        .connect(addr1)
    ...
    it('Should fail if tried to unstake before 2 hours', async () => {
      await hardhatUSDC
        .connect(addr1)
        .approve(hardhatStaking.address, 555 * 10 ** 6);
      await hardhatStaking.connect(addr1).stake(555 * 10 ** 6);
    ...
    it('Should be able to unstake after 2 hours', async () => {
      await hardhatUSDC
        .connect(addr1)
        .approve(hardhatStaking.address, 555 * 10 ** 6);
      await hardhatStaking.connect(addr1).stake(555 * 10 ** 6);
    ...
    it('Should fail if tried to unstake more than the amount that was put in even after 2 hours', async () => {
      await hardhatUSDC
        .connect(addr1)
        .approve(hardhatStaking.address, 555 * 10 ** 6);
      await hardhatStaking.connect(addr1).stake(555 * 10 ** 6);
  ...
  describe('Owner Related', () => {
    it('Should allow the owner of the contract to withdraw funds into treasury account', async () => {
      await hardhatUSDC
        .connect(addr1)
        .approve(hardhatStaking.address, 1999 * 10 ** 6);
      await hardhatStaking.connect(addr1).stake(1999 * 10 ** 6);
    ...
    it('Should fail if called by an account other than the owner', async () => {
      ...
      await expect(
        hardhatStaking.connect(addr1).collectFees(),
      ).to.be.revertedWith('Unauthorized Access');
    });

    it('Should allow owner to change the treasury address', async () => {
    ...
    it('Should fail if called by an account other than the owner', async () => {
      await expect(
        hardhatStaking.connect(addr1).changeTreasuryAddress(addr2.address),
      ).to.be.revertedWith('Unauthorized Access');
    });
  });
});
