...
describe("Staking V2", function () {
  ...
  describe("Staking Related", function () {
    ...
    it("Should fail if tried to unstake before 2 hours", async function () {
      ...
      await expect(
        hardhatStaking.connect(addr1).unStake(552.78 * 10 ** 6)
    ...
    it("Should be able to unstake after 2 hours", async function () {
      ...
      expect(await hardhatStaking.connect(addr1).unStake(552.78 * 10 ** 6));
    });
  ...
  describe("Access Roles", function () {
    it("Admin of TreasuryRole must be the default admin", async function () {
      await expect(await hardhatStaking.getRoleAdmin(treasuryRole)).to.be.equal(
    ...
    it("Default Admin should be able to grant new treasuryRole", async function () {
      ...
      expect(await hardhatStaking.hasRole(treasuryRole, addrs[0].address)).to.be
        .true;
    });
    ...
    it("Default Admin should be able to revoke the current treasury role", async function () {
      ...
      expect(await hardhatStaking.hasRole(treasuryRole, addr1.address)).to.be
        .false;
    });

    it("No other account should be able to revoke the current minter and pauser", async function () {
      await expect(
        hardhatStaking.connect(addrs[0]).revokeRole(treasuryRole, addr1.address)
      ).to.be.revertedWith(
        `VM Exception while processing transaction: reverted with reason string 'AccessControl: account ${addrs[0].address.toLowerCase()} is missing role ${defaultAdmin}`
      );
    });
    ...
    it("Default admin should be able to renounce defaultAdmin role", async function () {
      ...
      expect(await hardhatStaking.hasRole(defaultAdmin, owner.address)).to.be
        .false;
    });
  ...
  describe("Fees Related", function () {
    it("TreasuryRole address should be able to change the treasury address", async function () {
      ...
      expect(await hardhatStaking.treasury()).to.be.equal(addr3.address);
    });
    ...
    it("Any other address shouldn't be able to call the function to collect fees", async function () {
      await expect(hardhatStaking.collectFees()).to.be.revertedWith(
        `VM Exception while processing transaction: reverted with reason string 'AccessControl: account ${owner.address.toLowerCase()} is missing role ${treasuryRole}'`
      );
    });
  });
