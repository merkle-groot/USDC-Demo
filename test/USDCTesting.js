...
describe('USDC Contract', () => {
  ...
  describe('Transactions', () => {
    it('Should be able to send tokens between accounts', async () => {
    ...
    it('Should be able to approve USDC to another account', async () => {
    ...
    it('Should be able to spend the approved USDC', async () => {
      await hardhatUSDC.approve(addr1.address, 1000 * 10 ** 6);
    ...
    it('Should fail if tried to spend more than the approved amount', async () => {
      await hardhatUSDC.approve(addr1.address, 1000 * 10 ** 6);
      ...
      ).to.be.revertedWith('Unapproved tx');
    });
  });
  ...
  describe('Ownership', () => {
    ...
    it('Should fail if any other account tried to transfer ownership', async () => {
      await expect(
        hardhatUSDC.connect(addr1).changeOwner(addr1.address),
      ).to.be.revertedWith('Unauthorized Access');
      await expect(
        hardhatUSDC.connect(addr1).changeOwner(addr2.address),
      ).to.be.revertedWith('Unauthorized Access');
    });
    ...
    it('Should fail if any other account tried to renounce ownership', async () => {
      await expect(
        hardhatUSDC.connect(addr1).renounceOwnership(),
      ).to.be.revertedWith('Unauthorized Access');
    });
  });

  describe('Pausable', () => {
    it('Should be able to pausse txs', async () => {
      await hardhatUSDC.pause();
      await expect(
        hardhatUSDC.transfer(addr1.address, 1000 * 10 ** 6),
      ).to.be.revertedWith('SC is Paused');
    });

    it('Should fail if any other account tried to pause txs', async () => {
      await expect(hardhatUSDC.connect(addr1).pause()).to.be.revertedWith(
        'Unauthorized Access',
      );
    });

    it('Should be able to unpause the txs', async () => {
      await hardhatUSDC.pause();
      await expect(
        hardhatUSDC.transfer(addr1.address, 1000 * 10 ** 6),
      ).to.be.revertedWith('SC is Paused');
    ...
    it('Should fail if any other account tried to unpause the txs', async () => {
      await hardhatUSDC.pause();
      await expect(
        hardhatUSDC.transfer(addr1.address, 1000 * 10 ** 6),
      ).to.be.revertedWith('SC is Paused');

      await expect(hardhatUSDC.connect(addr1).unPause()).to.be.revertedWith(
        'Unauthorized Access',
      );
    });
  });
  ...
  describe('Mintable', () => {
    it('Should be able to mint more USDC into existence', async () => {
